...
calcSI <- function(Marker, Pedigree, genoPhased, nCore = NULL) {
  ...
  assert_set_equal(
    colnames(Marker),
    c("MarkerID", "Chr", "Map", "MarkerEff.a", "MarkerEff.d")
  )
  ...
  if (!is.null(nCore)) {
    if (OS == "Windows" & nCore > 1L) {
      warning(
        "Parallel calculation is not fully supported on Windows. nCore has been set to 1"
      )
      nCore <- 1L
    }
  ...
  for (i in 1:nIndiv) {
    ...
    if (sum(unlist(cross) %in% rownames(genoPhased)) == 3) {
      ...
      oDf <- data.frame(
        m = as.numeric(substr(offspring, 1, 1)),
        f = as.numeric(substr(offspring, 3, 3))
      )
      sDf <- data.frame(
        m = as.numeric(substr(seed, 1, 1)),
        f = as.numeric(substr(seed, 3, 3))
      )
      pDf <- data.frame(
        m = as.numeric(substr(pollen, 1, 1)),
        ...
        varCovList <- mclapply(
          ...
          function(chr) {
            ...
            varCov2 <- varCov_s *
              ...
              1 /
                4 *
                ((zyg_s %o% zyg_s) * varCov_s + (zyg_p %o% zyg_p) * varCov_p)
            varCov3 <- 1 /
              2 *
              (varCov_s *
                zyg_s[col(varCov_s)] +
                varCov_p * zyg_p[col(varCov_p)])

            markerName <- list(
              colnames(genoPhased)[idx],
              colnames(genoPhased)[idx]
            )
            dimnames(varCov_s) <- dimnames(varCov_p) <- dimnames(
              varCov1
            ) <- dimnames(varCov2) <- dimnames(varCov3) <- markerName
        ...
        vgListNew <- Reduce(
          ...
          mclapply(
            ...
            function(chr) {
              ...
              gEffect_mid.d <- ((pDf[idx, ]$zygote +
                sDf[idx, ]$zygote -
                pDf[idx, ]$zygote * sDf[idx, ]$zygote) %*%
                mEffectDom) /
                2
              ...
              Vgd <- t(mEffectDom) %*%
                (varCovSelect1 + 4 * varCovSelect2 - 4 * varCovSelect3) %*%
                mEffectDom
              Vgad <- 2 *
                t(mEffectAdd) %*% varCovSelect1 %*% mEffectDom -
                4 * t(mEffectAdd) %*% varCovSelect3 %*% mEffectDom
              Vg <- Vga + Vgd + Vgad
...
calcMarkerEff <- function(
  ...
  if (Model %in% c("Ridge", "Lasso")) {
    ...
    result <- do.call(BGLR, c(list(y = phenoSelect, ETA = Eta), args3))
  ...
  MarkerEstim$coefDetermin <- 1 -
    sum((phenoSelect - yEst)^2) / sum((phenoSelect - mean(phenoSelect))^2)
  ...
  rownames(MarkerEstim$mEffect$Add) <- rownames(
    MarkerEstim$mEffect$Dom
  ) <- colnames(Genotype)
